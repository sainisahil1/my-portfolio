...
const ExperienceCard = ({details}: {details: WorkDetails}) => {
    return(
        <div className="flex flex-row gap-20 skillcard items-center bg-white rounded-2xl p-8 w-2/3 hover:scale-[1.01] transition-transform">

              {/* Left Content */}
              <div className="flex-1">
                <h2 className="text-2xl font-bold mb-2"></h2>
                <h2 className="text-xl font-bold">{details.role}</h2>
                <p className="text-gray-500">{details.company} · {details.duration.from} – {details.duration.to}</p>
